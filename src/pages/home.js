...
export default function Home() {
    ...
    return (<>
        <div className="container-fluid p-0">
            ...
            {filter ? <Allpictures items={filteredPosts} username={user.username} /> : id ? <Allpictures items={filteredByArtistMatch} username={user.username}/> : post && user ? <Allpictures items={post} username={user.username}/>: post ? <Allpictures items={post} /> : <>Loading....</>}
            </div>
        </div>
    </>)
}
