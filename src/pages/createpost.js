...
export default function Createpost() {
  ...
  useEffect(() => {
    axios.post(API_URL + '/status', { token: localStorage.getItem('token') }).then((res) => {
      setuser(res.data)
      axios.get(`${API_URL}/bank/${res.data.username}`).then(() => sethaveaccount(true)).catch(error => {
        console.log(error);
      });
    }).catch(error => {
      console.log(error);
        ...
        <div className="col-12 col-md-8  d-flex flex-wrap justify-content-center justify-content-lg-start  gap-2">
          ...
          <button className="btn btn-dark w-100 mt-4" onClick={() => {
            const data = {
              ...
              nameaccount: nameaccount.current.value,
              bank: bank.current.value,
              number: number.current.value
            }
            axios.post(`${API_URL}/bank`, data)
            sethaveaccount(true)
          }}>Save</button></>
          }
        </div>
